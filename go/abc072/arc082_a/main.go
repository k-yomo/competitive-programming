...
func main() {
	...
	possibleXNums := map[int]bool{}
	for _, num := range a {
		numCountMap[num] += 1
		possibleXNums[num-1] = true
		possibleXNums[num] = true
		possibleXNums[num+1] = true
	}

	var max int
	for num := range possibleXNums {
		xCount := numCountMap[num-1] + numCountMap[num] + numCountMap[num+1]
		if xCount > max {
			max = xCount
		}
	}
