...
func main() {
	...
	n := io.ScanInt()
	pillarHeights := io.ScanInts(n)

	costs := []int{0, AbsDiff(pillarHeights[1], pillarHeights[0])}
	for i := 2; i < n; i++ {
		a := costs[i-2] + AbsDiff(pillarHeights[i], pillarHeights[i-2])
		b := costs[i-1] + AbsDiff(pillarHeights[i], pillarHeights[i-1])
		costs = append(costs, Min(a, b))
	}
