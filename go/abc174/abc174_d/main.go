...
func main() {
	...
	n := io.ScanInt()
	c := strings.Split(io.ScanString(), "")
	var redCount, needChangeWhiteCount int
	isAllWhite := true
	for i := n-1; i >= 0; i-- {
		if c[i] == "W" {
			if !isAllWhite {
				needChangeWhiteCount++
			}
		}
		if c[i] == "R" {
			redCount++
	...
	if needChangeWhiteCount == 0 {
		fmt.Println(0)
		return
	}

	isAllRed := true
	var whiteCount, needChangeRedCount int
	for i := 0; i < n; i++ {
		if c[i] == "R" {
			if !isAllRed {
				needChangeRedCount++
	...
	fmt.Println(Min(needChangeWhiteCount, needChangeRedCount))
}
