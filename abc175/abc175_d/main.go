...
func main() {
	...
	sortedC := append([]int{}, c...)
	sort.Ints(sortedC)
	maxScore := sortedC[n-1]

	for i := 0; i < n; i++ {
		curIndex := i
		visitedIndexMap := map[int]*lastVisited{}
		var score, curMax, count int
		for {
			nextIndex := p[curIndex] - 1
			if visitedIndexMap[nextIndex] >= 0 {
				break
			}
			score += c[nextIndex]
			if count == 0 || score > curMax {
				curMax = score
			}
			count++
			if count >= k {
			...
			visitedIndexMap[nextIndex] = &lastVisited{
				score: ,
				count: 0,
			}
			curIndex = nextIndex
