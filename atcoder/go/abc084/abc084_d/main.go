...
func main() {
	...
	q := io.ScanInt()
	isLike2017Map := map[int]bool{}
	var max int
	for i := 1; i < int(math.Pow10(5)); i += 2 {
		if IsPrime(i) && (isLike2017Map[(i+1)/2] || IsPrime((i+1)/2)) {
			isLike2017Map[i] = true
			if i > max {
				max = i
			}
		}
	...
	cumulativeSumArray := make([]int, max+1)
	cumulativeSumArray[1] = 1
	for i := 3; i < max+1; i += 2 {
		sum := cumulativeSumArray[i-2]
		if  isLike2017Map[i] {
			sum+=1
		}
	...
	for i := 0; i < q; i++ {
		...
		io.Println(cumulativeSumArray[r] - cumulativeSumArray[l-1])
	}
}
