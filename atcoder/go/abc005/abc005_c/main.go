...
func main() {
	...
	t, n := io.ScanInt2()
	takiyakiReadyTimes := io.ScanInts(n)
	m := io.ScanInt()
	customerVisitTimes := io.ScanInts(m)

	curTakiyakiIdx := 0
	for _, visitTime := range customerVisitTimes {
		if curTakiyakiIdx > n-1 {
			fmt.Println("no")
			return
		}
		if takiyakiReadyTimes[curTakiyakiIdx] > visitTime {
			fmt.Println("no")
			return
		}
		for {
			if (visitTime - takiyakiReadyTimes[curTakiyakiIdx]) <= t {
				curTakiyakiIdx++
				break
			}
			curTakiyakiIdx++
