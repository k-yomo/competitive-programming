...
func main() {
	...
	if whiteLaneHeight == -1 {
		fmt.Println(":(")
		return
	}

	if IsReachable(graph, 0, 0, whiteLaneHeight, w-1) && IsReachable(graph, whiteLaneHeight, w-1, h-1, w-1) {
		fmt.Println("Yay!")
	} else {
		fmt.Println(":(")
	}
...
func IsReachable(graph [][]string, curY, curX, goalY, goalX int) bool {
	if curY == len(graph) || curX == len(graph[0]) || graph[curY][curX] == "#" {
		return false
	}
	if curY == goalY && curX == goalX {
		return true
	}
	if IsReachable(graph, curY, curX+1, goalY, goalY) {
		return true
	}
	if IsReachable(graph, curY+1, curX, goalY, goalX) {
		return true
	}
	return false
}
