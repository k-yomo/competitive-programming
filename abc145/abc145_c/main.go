...
func main() {
	...
	towns := make([]*axis, n)
	for i := 0; i < n; i++ {
		towns[i] = &axis{x: io.ScanInt(), y: io.ScanInt()}
	}
	combinations := permutation(towns)
	var sum float64
	for _, comb := range combinations {
		for i := 0; i < n-1; i++ {
			left := math.Pow(float64(comb[i].x - comb[i+1].x), 2)
			right := math.Pow(float64(comb[i].y - comb[i+1].y), 2)
			sum += math.Sqrt(left + right)
		}
	}
	fmt.Println(sum / float64(len(combinations)))
}

func remove(ar []*axis, i int) []*axis {
	tmp := make([]*axis, len(ar))
	copy(tmp, ar)
	return append(tmp[0:i], tmp[i + 1:]...)
}
func permutation(ar []*axis) [][]*axis {
	var result [][]*axis
	if len(ar) == 1 {return append(result, ar)}
	for i, a := range ar {
		for _, b := range permutation(remove(ar, i)) {
			result = append(result, append([]*axis{a}, b...))
		}
	}
	return result
}
