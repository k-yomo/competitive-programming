...
func main() {
	...
	var maxPopulationTown string
	var totalPopulation int
	for i := 0; i < n; i++ {
		townName, population := io.ScanString(), io.ScanInt()
		if maxPopulation < population {
			maxPopulation = population
			maxPopulationTown = townName
		}
		totalPopulation += population
	}
	if maxPopulation > totalPopulation/2 {
		io.Println(maxPopulationTown)
	} else {
		io.Println("atcoder")
	}
}
